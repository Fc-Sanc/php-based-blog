...
function installBlog(PDO $pdo)
{
    ...
    // A Security measure, to avoid anyone resting the database if it already exists
    if (is_readable($database) && filesize($database) > 0) {
        $error = 'Please delete the existing database manually before installing it afreah';
    }

    // Create an empty file for the database
    if (!$error) {
        $created_ok = @touch($database);
        if (!$created_ok) {
            $error = sprintf(
...
function createUser(PDO $pdo, $username, $length = 10)
{
    ...
    // We're storing the password in plaintext, will fin that later
    if (!$error) {
